...
describe("Property 4: Evaluation result structure matches type", () => {
    ...
    const rangeResultArbitrary = fc.record({
        value: fc.integer({ min: 0, max: 100 }),
        min: fc.constant(0),
        max: fc.constant(100),
        rationale: fc.string({ minLength: 1, maxLength: 500 }),
    }).filter((result) => result.value >= result.min && result.value <= result.max);
    ...
    it("should validate mixed evaluation types maintain structure integrity", async () => {
        const mixedResultArbitrary = fc.oneof(
            ...
            fc.record({ type: fc.constant("comments"), result: commentsResultArbitrary }),
            fc.record({ type: fc.constant("code_examples"), result: codeExamplesResultArbitrary })
        );
            ...
            fc.property(mixedResultArbitrary, (evaluation) => {
                ...
                switch (evaluation.type) {
                    ...
                    case "range":
                        ...
                        expect(evaluation.result.value).toBeGreaterThanOrEqual(evaluation.result.min);
                        expect(evaluation.result.value).toBeLessThanOrEqual(evaluation.result.max);
                        break;
                    case "comments":
