    ...
    it("should store complete result data for any evaluation type", async () => {
        await fc.assert(
            fc.asyncProperty(
                ...
                async (userData, repoData, rubricData, rubricItem, evaluationResult) => {
                    const t = convexTest(schema, modules);
    ...
    it("should mark analysis as completed when all items are processed", async () => {
        await fc.assert(
            fc.asyncProperty(
                ...
                fc.array(fc.constantFrom("completed", "failed"), { minLength: 2, maxLength: 5 }),
                async (userData, repoData, rubricData, rubricItems, itemStatuses) => {
                    ...
                    for (let i = 0; i < rubricItemIds.length; i++) {
                        await t
                            ...
                            .mutation(api.analyses.updateItemResult, {
                                ...
                                status: statuses[i],
                                result: statuses[i] === "completed" ? { value: true } : undefined,
                    ...
                    expect(analysis!.completedItems + analysis!.failedItems).toBe(items.length);
