...
async function createTestUser(
    ...
    return await t.mutation(api.users.syncUser, {
        clerkId: id,
        email: `${id}@test.com`,
        name: "Test User",
    });
    ...
    it("should persist rubric with matching data and empty items list", async () => {
        await fc.assert(
            fc.asyncProperty(rubricDataArbitrary, async (rubricData) => {
                ...
                // Create the rubric
                const rubricId = await t.mutation(api.rubrics.createRubric, {
                    userId,
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (rubricData1, rubricData2) => {
                    ...
                    // Create two rubrics
                    const rubricId1 = await t.mutation(api.rubrics.createRubric, {
                        userId,
                        name: rubricData1.name,
                        description: rubricData1.description,
                    });

                    const rubricId2 = await t.mutation(api.rubrics.createRubric, {
                        userId,
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (userRubricsData) => {
                    ...
                    // Create user rubrics
                    for (const rubricData of userRubricsData) {
                        await t.mutation(api.rubrics.createRubric, {
                            userId,
                            name: rubricData.name,
                            description: rubricData.description,
                        });
                    ...
                    const intersection = new Set([...userIds].filter((id) => templateIds.has(id)));
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (templateId) => {
                    ...
                    const originalTemplate = SYSTEM_TEMPLATES.find((st) => st.id === templateId)!;
                    expect(templateWithItems!.items.length).toBe(originalTemplate.items.length);
                },
            ),
            { numRuns: 50 },
        );
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (templateId) => {
                    ...
                    expect(firstTemplate!.systemTemplateId).toBe(secondTemplate!.systemTemplateId);
                },
            ),
            { numRuns: 30 },
        );
            ...
            fc.asyncProperty(
                ...
                async (templateId, customName) => {
                    ...
                    if (customName) {
                        ...
                        expect(duplicatedRubric!.name).toBe(`${originalTemplate!.name} (Copy)`);
                    }

                    // Property: All items are copied
                    expect(duplicatedRubric!.items.length).toBe(originalTemplate!.items.length);
                    ...
                    for (let i = 0; i < originalTemplate!.items.length; i++) {
                        ...
                        expect(duplicatedItem.evaluationType).toBe(originalItem.evaluationType);
    ...
    it("should reject duplication of non-system templates", async () => {
        await fc.assert(
            fc.asyncProperty(rubricDataArbitrary, async (rubricData) => {
                ...
                const userRubricId = await t.mutation(api.rubrics.createRubric, {
                    userId,
                    name: rubricData.name,
                    description: rubricData.description,
                });
