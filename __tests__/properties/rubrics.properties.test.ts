...
describe("Property 3: Rubric item validation", () => {
    ...
    const nonRangeEvaluationTypeArbitrary = fc.constantFrom(
        ...
        "comments",
        "code_examples",
    ) as fc.Arbitrary<"yes_no" | "comments" | "code_examples">;
    ...
    const validItemDataArbitrary = fc.record({
        ...
        description: fc
            ...
            .filter((s) => s.trim().length > 0),
        evaluationType: nonRangeEvaluationTypeArbitrary,
    });
...
/**
 * **Feature: ai-code-review, Property 4.1: Range item requires guidance text**
 * *For any* rubric item with evaluation type "range", the item SHALL only be
 * stored if it contains a non-empty `rangeGuidance` string describing when
 * each score level should be selected.
 * **Validates: Requirements 2.4**
 */
describe("Property 4.1: Range item requires guidance text", () => {
    it("should store range items with valid guidance text", async () => {
        await fc.assert(
            fc.asyncProperty(
                rubricDataArbitrary,
                fc
                ...
                async (rubricData, itemName, itemDescription, rangeGuidance, maxVal) => {
                    const t = convexTest(schema, modules);
                    const { clerkId } = await createTestUser(t);
                    ...
                    // Property: Range item with valid guidance should be stored
                    const itemId = await t.mutation(api.rubrics.addRubricItem, {
                        rubricId,
                        name: itemName,
                    ...
                    const item = await t.query(api.rubrics.getRubricItem, { itemId });
                    expect(item).not.toBeNull();
                    expect(item?.evaluationType).toBe("range");
                    expect(item?.config.rangeGuidance).toBe(rangeGuidance);
                },
            ),
            { numRuns: 100 },
        );
    ...
    it("should reject range items without guidance text", async () => {
        await fc.assert(
            fc.asyncProperty(
                rubricDataArbitrary,
                fc
                ...
                fc
                    .string({ minLength: 1, maxLength: 500 })
                    .filter((s) => s.trim().length > 0),
                async (rubricData, itemName, itemDescription) => {
                    const t = convexTest(schema, modules);
                    const { clerkId } = await createTestUser(t);
                    ...
                    // Property: Range item without guidance should be rejected
                    await expect(
                        t.mutation(api.rubrics.addRubricItem, {
                            rubricId,
                            name: itemName,
    ...
    it("should reject range items with empty or whitespace-only guidance", async () => {
        await fc.assert(
            fc.asyncProperty(
                rubricDataArbitrary,
                fc
                ...
                fc
                    ...
                    .filter((s) => s.trim().length > 0),
                fc.constantFrom("", "   ", "\t", "\n", "  \t\n  "),
                async (rubricData, itemName, itemDescription, emptyGuidance) => {
                    const t = convexTest(schema, modules);
                    const { clerkId } = await createTestUser(t);
                    ...
                    // Property: Range item with empty guidance should be rejected
                    await expect(
                        t.mutation(api.rubrics.addRubricItem, {
                            rubricId,
                            name: itemName,
    ...
    it("should reject range item updates without guidance text", async () => {
        await fc.assert(
            fc.asyncProperty(
                rubricDataArbitrary,
                fc
                ...
                fc
                    .string({ minLength: 1, maxLength: 500 })
                    .filter((s) => s.trim().length > 0),
                async (rubricData, itemName, itemDescription, rangeGuidance) => {
                    const t = convexTest(schema, modules);
                    const { clerkId } = await createTestUser(t);
                    ...
                    // Create a valid range item first
                    const itemId = await t.mutation(api.rubrics.addRubricItem, {
                        rubricId,
                        name: itemName,
                        description: itemDescription,
                    ...
                    // Property: Updating range item to remove guidance should be rejected
                    await expect(
                        t.mutation(api.rubrics.updateRubricItem, {
                            itemId,
                            config: {
                                minValue: 1,
                                maxValue: 10,
    ...
    it("should not require guidance for non-range evaluation types", async () => {
        const nonRangeTypes = fc.constantFrom(
            "yes_no",
            "comments",
            "code_examples",
        ) as fc.Arbitrary<"yes_no" | "comments" | "code_examples">;
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                fc
                    ...
                    .filter((s) => s.trim().length > 0),
                nonRangeTypes,
                async (rubricData, itemName, itemDescription, evalType) => {
                    const t = convexTest(schema, modules);
                    const { clerkId } = await createTestUser(t);
                    ...
                    // Property: Non-range items should not require guidance
                    const itemId = await t.mutation(api.rubrics.addRubricItem, {
                        rubricId,
                        name: itemName,
                        description: itemDescription,
                        evaluationType: evalType,
                        config: {}, // No rangeGuidance needed
                    });
                    ...
                    const item = await t.query(api.rubrics.getRubricItem, { itemId });
                    expect(item).not.toBeNull();
                    expect(item?.evaluationType).toBe(evalType);
                },
            ),
            { numRuns: 100 },
        );
